...
def check_import_status():
    ...
    for station_id in SELECTED_STATIONS:
        try:
            ...
            count = session.query(func.count(DailyMeasurement.date)).filter_by(station_id=station_id).scalar() or 0
            total_records += count
            
            if station:
                status = "✓"
                print(f"{status} {station_id} | {station.name:40s} | {count:6,d} records | {station.start_date} to {station.end_date}")
            else:
    ...
    print("="*80)
    print(f"TOTAL: {len(SELECTED_STATIONS)} stations | {total_records:,d} total measurements")
    print("="*80 + "\n")
