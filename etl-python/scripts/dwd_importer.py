...
class DWDImporter:
    ...
    def download_station_data(self, station_id: str) -> Optional[pd.DataFrame]:
        ...
        """
        try:
            logger.info(f"Versuche echte DWD-Daten für Station {station_id} zu laden...")
            
            # Versuche direkten Download von DWD Server
            data = self._download_real_dwd_data(station_id)
            
            if data is not None and not data.empty:
                logger.success(f"{len(data)} echte Messungen für Station {station_id} vom DWD Server geladen")
                return data
            
            # Fallback auf erweiterte simulierte Daten
            logger.warning(f"Direkter DWD-Download für Station {station_id} fehlgeschlagen, verwende erweiterte simulierte Daten")
            return self._create_extended_sample_data(station_id)
            
        except Exception as e:
            logger.error(f"Download für Station {station_id} fehlgeschlagen: {e}")
            logger.warning(f"Falle zurück auf einfache Beispieldaten für Station {station_id}")
    ...
    def _download_real_dwd_data(self, station_id: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if historical_data is not None and not historical_data.empty:
                logger.success(f"Historische DWD-Daten für Station {station_id} geladen: {len(historical_data)} Datensätze")
            ...
            logger.error(f"Fehler beim Download von DWD-Daten für Station {station_id}: {e}")
            return None
    ...
    def _download_historical_dwd_data(self, station_id: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(f"Konnte historische Daten-Liste nicht laden: Status {response.status_code}")
                return None
            ...
            pattern = re.compile(f'tageswerte_KL_{station_id}_\\d{{8}}_\\d{{8}}_hist\\.zip')
            zip_files = []
            ...
            if not zip_files:
                logger.warning(f"Keine historischen ZIP-Dateien für Station {station_id} gefunden")
                return None
            ...
            logger.error(f"Fehler beim Laden historischer DWD-Daten für Station {station_id}: {e}")
            return None
    
    def _download_and_parse_dwd_zip(self, url: str, station_id: str) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            if cache_file.exists():
                ...
                if cache_age < max_cache_age:
                    logger.info(f"Verwende gecachte ZIP-Datei für Station {station_id} (Alter: {cache_age/86400:.1f} Tage)")
                    zip_content = cache_file.read_bytes()
                else:
                    logger.info(f"Cache zu alt ({cache_age/86400:.1f} Tage), lade neu...")
                ...
                # Lade von Server
                logger.info(f"Lade ZIP-Datei für Station {station_id} vom DWD-Server...")
                zip_content = self._download_zip_with_retry(url)
                if zip_content:
                    cache_file.write_bytes(zip_content)
                else:
    ...
    def _download_zip_with_retry(self, url: str, max_retries: int = 3) -> Optional[bytes]:
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Server antwortet mit Status {response.status_code} für {url}")
                    
            except requests.exceptions.Timeout:
                logger.warning(f"Timeout beim Download von {url} (Versuch {attempt + 1}/{max_retries})")
            except requests.exceptions.ConnectionError:
                logger.warning(f"Verbindungsfehler beim Download von {url} (Versuch {attempt + 1}/{max_retries})")
            except Exception as e:
                logger.warning(f"Fehler beim Download von {url}: {e} (Versuch {attempt + 1}/{max_retries})")
    ...
    def _parse_zip_content(self, zip_content: bytes, station_id: str) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            with zipfile.ZipFile(zip_buffer) as zip_file:
                ...
                product_files = [f for f in zip_file.namelist() if f.startswith('produkt_klima_tag')]
                
                if not product_files:
                    logger.warning(f"Keine Produkt-Datei in ZIP gefunden für Station {station_id}")
                    return None
                ...
                with zip_file.open(product_file) as csv_file:
                    ...
                    logger.debug(f"Verfügbare Spalten in DWD-Daten für Station {station_id}: {list(df.columns)}")
                    
                    # Transformiere DWD-Daten in unser Format
                    return self._transform_dwd_to_internal_format(df, station_id)
                    
        except Exception as e:
            logger.error(f"Fehler beim Parsen von ZIP-Inhalt für Station {station_id}: {e}")
            return None
    
    def _transform_dwd_to_internal_format(self, df: pd.DataFrame, station_id: str) -> pd.DataFrame:
        """
        ...
        required_columns = [
            'station_id', 'date', 'temp_max', 'temp_min', 'temp_mean',
            'precipitation', 'sunshine', 'snow_depth', 'quality_flags',
    ...
    def _get_station_metadata(self, station_id: str) -> Dict[str, any]:
        ...
        station_metadata = {
            ...
            '01072': {'name': 'Dresden-Klotzsche', 'lat': 51.13, 'lon': 13.75, 'elevation': 227},
            ...
            '01297': {'name': 'Stuttgart-Echterdingen', 'lat': 48.69, 'lon': 9.22, 'elevation': 371},
            '01303': {'name': 'Saarbrücken-Ensheim', 'lat': 49.22, 'lon': 7.11, 'elevation': 320},
            '01346': {'name': 'Rostock-Warnemünde', 'lat': 54.18, 'lon': 12.08, 'elevation': 4},
    ...
    def _create_extended_sample_data(self, station_id: str) -> pd.DataFrame:
        ...
        logger.info(f"{len(data)} erweiterte Beispieldaten für Station {station_id} generiert")
        return data
    ...
    def _import_measurements(self, station_id: str, session):
        ...
        existing_dates = set()
        existing_records = session.query(DailyMeasurement.date).filter_by(station_id=station_id).all()
        if existing_records:
